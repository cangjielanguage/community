...
1. **归口管理**：代码仓须归属于特定项目。建仓申请由所属项目向对应项目管理委员会（以下简称“PMC”）提交。
2. **职责明确**：建仓申请须明确仓库的责任团队（Team或社区运营办公室），经 PMC 评审通过后，由授权管理员在[Cangjie-SIG](https://gitcode.com/Cangjie-SIG)组织下执行建仓。
3. **分级审批**：
   - **核心仓库**：凡进入仓颉语言社区版本的仓库，必须经 PMC 评审后方可建立。
   - **非核心仓库**：不进入仓颉语言社区版本的仓库，可在 Team 评审通过后先行建立，但须向 PMC 备案并同步。
   - **权限下放**：PMC 可根据实际治理需要，在其职权范围内下放仓库评审权限。

## 三、 仓库配置与命名规范

### **1. 命名与基础设置**

- **命名准则**：核心仓库原则上采用 `cangjie_` 前缀，使用小写字母命名，单词间以底划线 `_` 分隔。例如：`cangjie_multiplatform_interop`。
...
- **新建申请阶段**：详见本文第二部分，[“代码仓建立与准入”](#二、 代码仓建立与准入)。
- **孵化预审阶段**：详见本文第二部分，[“代码仓建立与准入”](#二、 代码仓建立与准入)。
- **准出终审阶段**：
