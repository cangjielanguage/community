# team_docs
## Docs Team工作目标和范围

### 工作目标
 负责仓颉语言文档的社区需求规划和梳理对应领域的共建需求梳理。

### 工作范围
- 文档规划和信息架构设计、文档版本生命周期管理，风格指南等质量保证和标准化规范制定、文档贡献流程建设、文档维护更新、审核文档、响应并处理社区文档问题反馈。



### 工作交付件及工作计划



## Team成员

### Leader
- 赵文远[@huster_zwy](https://gitcode.com/huster_zwy) / 朱艳婷 [@amy_mayun](https://gitcode.com/amy_mayun)



### 会议
- **会议时间**：每周五 16:30
- **会议申报**：[Team_Docs Meeting Proposal](https://gitcode.com/Cangjie/community/blob/main/team/team_docs/meetings/meeting-notices.md)
- **会议链接**: Welink
- **会议通知**: 请[订阅](https://cangjie-lang.cn/pages/maillist)邮件列表 [docs@cangjie-lang.cn ](mailto:docs@cangjie-lang.cn) 获取会议链接
- **会议纪要**：[查看历史会议纪要](https://gitcode.com/Cangjie/community/blob/main/team/team_docs/meetings/)



### 联系方式

- 邮件列表： docs@cangjie-lang.cn 